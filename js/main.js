  ...
  function windowLoadInit() {
    ...
    jQuery('form.contact-form').on('submit', function (e) {
      ...
      var ajax = jQuery.post($form.attr('action'), request).then(function () {
        jQuery($form)
          ...
          .parent()
          .append(
            '<span style="padding-top: 10px; font-size: 24px" class="contact-form-respond highlight"> <i class="fas fa-check-circle"></i>   Formulario enviado </span>'
          );
        //cleaning form
    ...
    if (jQuery().tweet) {
      jQuery('.twitter').tweet({
        ...
        template:
          '<span class="tweet_text">{tweet_text}</span><span class="darklinks black">- {time}</span>',
      });
    }
    ...
    jQuery('.flex-control-nav.owl-carousel').each(function () {
      var $owl = jQuery(this);
      var $owlFaces = $owl
        .parents('.testimonials_flexslider')
        .prev('.testimonials_faces');
      ...
      $owl1
        ...
        .on('change.owl.carousel', function (event) {
          if (event.namespace && event.property.name === 'position') {
            ...
            initProgressbar(
              jQuery($owl2).find('.active .progress .progress-bar')
            );
          }
        });
    });
      ...
      $googleMaps.each(function () {
        ...
        var styles = [
          ...
          },
          {
            featureType: 'water',
            elementType: 'all',
            stylers: [{ color: '#0095b3' }, { visibility: 'on' }],
          },
        ];
